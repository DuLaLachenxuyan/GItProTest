...
import com.trw.generator.IncrementIdGenerator;
import com.trw.generator.UniqueIdGenerator;
...
//	}
     ...
     * @return
     */
	public static CodeSunVo mapToVo(TrwTDict dic) {
		CodeSunVo newVo = new CodeSunVo();
		newVo.setDiccode(dic.getDiccode());
		newVo.setDiclabel(dic.getDiclabel());
		newVo.setMax(dic.getCmax());
		newVo.setMin(dic.getCmin());
		newVo.setRemark(dic.getRemark());
		newVo.setDictype(dic.getDictype());
		newVo.setDickey(dic.getDickey());
		newVo.setPkey(dic.getPkey());
		return newVo;
	}
    /**
     * 是否添加过，返回对象
     ...
     * @return
     */
	public static CodeVo has(TrwTDict dic, List<CodeVo> list) {
		int size = list.size();
		for(int i=0;i<size;i++) {
			if(dic.getDictype().equals(list.get(i).getDictype())) {
				return list.get(i);
			}
		}
		return null;
	 ...
	 * @return
	 */
	public static String generateId(String prifex) {
		return prifex +UniqueIdGenerator.getInstance(IncrementIdGenerator.getServiceId()).nextId();
	}
}